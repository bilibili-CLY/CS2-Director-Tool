...
public class KillReplayViewModel : ViewModelBase
{
    ...
    private string _replaySourceName = string.Empty;
    ...
    /// <summary>保护击杀时间戳记录与录制会话状态。</summary>
    private readonly object _killLock = new();

    /// <summary>指示录制会话是否处于活动状态；只有活动会话内的击杀才会被记录。</summary>
        ...
        set
        {
            if (SetProperty(ref _isKillReplayEnabled, value))
            {
                ...
                if (value && !_canBeActive)
                    Log("已勾选启用击杀回放，但前置条件未满足，未真正启用");
                else
                    Log(value ? "已启用击杀回放" : "已取消启用击杀回放");
                if (!value)
                    StopKillReplayAsync();
            }
        }
    ...
    /// <summary>获取一个值，指示击杀回放功能既被用户启用，又被其前提条件允许运行。</summary>
    private bool IsEffectivelyEnabled => _isKillReplayEnabled && _canBeActive;

    /// <summary>获取一个值，指示当前是否有回放正在播放。</summary>
    public bool IsReplayPlaying => _isReplayPlaying;

    /// <summary>回放播放结束且游戏场景重新激活时发生。</summary>
    public event Action? ReplayPlaybackEnded;

    /// <summary>功能被停用时停止当前录制并重置回放状态。</summary>
    ...
    /// <summary>初始化 <see cref="KillReplayViewModel"/> 类的新实例。</summary>
    ...
    private void Log(string message) => _log.Log(LogCategory.Replay, message);

    /// <summary>刷新前提条件状态，并在保留用户复选框偏好的前提下，相应地启动或暂停击杀回放流程。</summary>
    public void RefreshPrerequisites()
    {
        ...
        if (prerequisitesMet)
            Log(_isKillReplayEnabled ? "前置条件已满足，击杀回放已恢复" : "前置条件已满足");
        else
        {
            Log("前置条件不满足，击杀回放已暂停（勾选状态保留，条件满足后自动恢复）");
            StopKillReplayAsync();
        }
    }
    ...
    private void OnKill(object? sender, GsiKillEventArgs e)
    {
        ...
        lock (_killLock)
        {
            ...
            {
                Log($"击杀 {e.KillerName}->{e.VictimName} 被跳过：录制未开始或不在当前录制会话内");
                return;
            }
        ...
        Log($"击杀: {e.KillerName} 击杀 {e.VictimName}（回合内 {secondsSinceRoundStart:F1}s，视角在: {observerName}）");
    }
    ...
    private async Task OnRoundStarted()
    {
        ...
        // freezetime -> live 的切换会触发两次本方法；一个回合只需要初始化一次。
        if (_isInRound)
            return;
    ...
    /// <summary>启动 OBS 录制（若尚未在录制）。</summary>
    ...
    private async Task OnRoundEnded()
    {
        ...
        try
        {
            ...
            Log($"回合结束，处理 {timestamps.Length} 个击杀镜头");
            IsProcessing = true;
            ...
            if (_recordingStartTime != default)
            {
                ...
                if (elapsedSinceLastKill < 1.0)
                {
                    ...
                    Log($"回合结束，最后击杀后仅 {elapsedSinceLastKill:F2}s，等待 {wait:F2}s 确保保留击杀后画面");
            ...
            try
            {
                ...
                Log($"开始剪辑 {clips.Count} 个片段 -> {outputFile}");
    ...
    private async Task OnClippingComplete(ClippingCompletedEventArgs e)
    {
        try
        {
            ...
            try
            {
                ...
                Log($"已在 '{replayScene}' 中设置源 '{sourceName}': {videoFile}");
            ...
            catch (Exception ex)
            {
                ...
                Log($"恢复录制失败: {ex.Message}");
                IsProcessing = false;
                return;
            }

            // 录制已（重新）启动，录制时间轴已归零；将击杀位置重新锚定到新的零点。
            lock (_killLock)
            {
                _recordingStartTime = DateTime.Now;
                _recordingActive = true;
